...
const verificationPrompt = ai.definePrompt({
  ...
  Verification Rules:
  1.  **Title Match:** The handwritten title in the image(s) must exactly match the "Official Task Title" provided above.
  2.  **Handwriting Check:** The content of the assignment must be handwritten. Reject any computer-typed or printed text.
  
  Your Decision Process:
  - If both rules are met, set isApproved to true and the reason to "Approved".
  - If the handwritten title does not match the official title, set isApproved to false and the reason to "Title mismatch".
  - If the assignment is not handwritten, set isApproved to false and the reason to "Not handwritten".
  - If the image is unclear, blank, or does not contain a recognizable assignment, set isApproved to false and the reason to "Invalid content".
